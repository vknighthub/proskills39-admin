...
await $`yarn --cwd /var/www/pixer-react/shop/`;

echo(chalk.blue("Running for shop with pm2"));
await $`pm2 --name shop-rest start yarn --cwd /var/www/pixer-react/shop -- run start`;

echo(chalk.blue("Install admin packages"));
await $`yarn --cwd /var/www/pixer-react/admin/`;

echo(chalk.blue("Running for admin with pm2"));
await $`pm2 --name shop-rest start yarn --cwd /var/www/pixer-react/admin -- run start`;
