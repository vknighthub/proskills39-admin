...
// let front_end_source_path = await question('Enter frontend.zip source path (ex: /home/../pixer-react/frontend.zip): ')
let front_end_source_path = "./frontend.zip";
echo("Uploading frontend.zip to server, Please wait...");
await $`scp ${front_end_source_path} ${username}@${ip_address}:/var/www/pixer-react`;
echo(chalk.green("Uploaded frontend.zip to server"));

await $`ssh -o StrictHostKeyChecking=no -l ${username} ${ip_address} "unzip /var/www/pixer-react/frontend.zip -d /var/www/pixer-react";`;
